          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {LEARNING_STEPS.map((step) => (
              <div key={step.id} className="relative flex flex-col items-center text-center">
                <div className="w-24 h-24 bg-white rounded-full border-4 border-slate-50 shadow-sm flex items-center justify-center mb-6 z-10 relative">
                  <div className="absolute top-0 right-0 bg-teal-600 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">
                    {step.number}
                  </div>
